...
export default function BaristaOnboarding() {
  ...
  const compressImage = (file, maxWidth = IMAGE_MAX_WIDTH) =>
  ...
  const uploadFile = async (bucket, userId, file, folder) => {
    ...
    if (shouldCompress) {
      const maxWidth =
        bucket === BUCKETS.profile ? PROFILE_MAX_WIDTH : IMAGE_MAX_WIDTH;
      uploadFileRef = await compressImage(file, maxWidth);
    }
